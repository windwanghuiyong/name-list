...
{
    static dispatch_once_t once;			// 线程

    dispatch_once(&once, ^{
        manager = [[self alloc] init];	// 初始化单例模式的 SQLManager 对象
        [manager createDataBaseTableIfNeeeded];
    });
    return manager;
}
...
-(void)createDataBaseTableIfNeeeded
{
    ...
    else
    {
        ...
        {
            NSLog(@"create studentName seccess");
        }
        sqlite3_close(db);
    }
}
...
- (StudentModel *)searchWithIdNum:(StudentModel *) model
{
    ...
    else
    {
        ...
        if (sqlite3_prepare_v2(db, [qsql UTF8String], -1, &statement, NULL) == SQLITE_OK)
        {
            ...
            {
                NSLog(@"found");
                // 提取: 语句对象, select 字段索引, 查 * 时会按建表示的顺序
